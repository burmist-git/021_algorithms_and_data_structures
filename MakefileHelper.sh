...
function create_conda_env_sh {
    ...
    if [ $wcl = 0 ]; then
	conda create --name $condaEvnName -y numpy notebook 
...
function install_from_conda_forge {
    conda search -c conda-forge $1
    echo "installing --> $1"
    conda install -y -c conda-forge $1
}
...
function install_packages_sh {
	...
	# pandas
	install_from_conda_forge "pandas=1.2.4"
